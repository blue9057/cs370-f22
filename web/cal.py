...
def parse(data):
    ...
    # questions
    if not os.path.exists("q.rst"):
        return

    with open("q.rst") as fd:
        qdict = {"xxx": 1}
        for l in fd:
            qdict[l.strip()] = 1
        for q in prep_qs:
            if q in qdict:
                continue
            print >> sys.stderr, "WARN: can't find %s in q.rst" % q
...
class CalWriter(object):
    ...
    def __emit(self, d, end, style, html):
        ...
        if d.weekday() < 5:
            ...
            if span != 1:
                attr += ' colspan="%d"' % span
            c = "".join("<br />\n    " + l for l in html)
            print '  <td %s><span class="date">%s</span>%s</td>' % (attr, s.capitalize(), c)
        if d.weekday() == 6:
            print "</tr>"
    ...
    def item(self, text = None, style = None):
        if text:
            ...
            for (link, _) in re.findall("(?s)\[(\S*)(?:\s*(.*?))\]", text):
                # XXX. haskish rules
                if not re.match("^https?://", link):
                    link = re.sub("^l/", "lec/", link)
                    link = re.sub("^r/", "readings/", link)
                    if not os.path.exists(link):
                        print >> sys.stderr, "WARN: can't find %s" % link
        else:
            html = []
        d = self.__start
        while d <= self.__end:
    ...
    def Lec(self, text):
        self.item("<b>LEC %d:</b> %s" % (self.__lec, text), "lecture")
        self.__lec += 1
        ...
        if text is not None:
            def translate_prep(line):
                ...
                if func == "read":
                    ...
                    if os.path.exists("readings/" + pdf):
                        sub += "[r/%s %s]" % (pdf, label)

                    for ext in ["pdf", "pptx", "ppt"]:
                        ppt = "%s-slides.%s" % (args[0], ext)
                        if os.path.exists("readings/" + ppt):
                            sub += " / [r/%s slides]" % (ppt)
                            break
                else:
                    assert False

                return re.sub(re_func, sub, line)

            preptext += ' %s' % translate_prep(text)
        if extra is not None:
            # HACK: self.__lec contains the next lecture number, so subtract 1.
            preptext += (' (<a href="q.html?q=%s&amp;lec=%d">Question</a>)' %
                         (extra, self.__lec - 1))

            self.prep_qs.append(extra)
...
if __name__ == '__main__':
    cal = file(sys.argv[1]).read()
    cal = cal.replace("{{LABS}}", lab.dump_cal())
    parse(cal)
